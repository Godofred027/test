import React from "react";
import Link from "next/link";
import { useRouter } from "next/router";
import NotificationDropdown from "@/components/Dropdowns/NotificationDropdown";
import UserDropdown from "@/components/Dropdowns/UserDropdown";

export default function Sidebar() {
  const [collapseShow, setCollapseShow] = React.useState("hidden");
  const router = useRouter();
  return (
    <>
      <nav className="md:left-0 md:block md:fixed md:top-0 md:bottom-0 md:overflow-y-auto md:flex-row md:flex-nowrap md:overflow-hidden shadow-xl bg-gray-200 flex flex-wrap items-center justify-between relative md:w-64 z-10 py-4 px-6">
        <div className="md:flex-col md:items-stretch md:min-h-full md:flex-nowrap px-0 flex flex-wrap items-center justify-between w-full mx-auto">
          {/* Toggler */}
          <button
            className="cursor-pointer text-black opacity-50 md:hidden px-3 py-1 text-xl leading-none bg-transparent rounded border border-solid border-transparent"
            type="button"
            onClick={() => setCollapseShow("bg-white m-2 py-3 px-6")}
          >
            <i className="fas fa-bars"></i>
          </button>
          {/* Brand */}
          <Link
            href="/dashboard"
            className="md:block text-left md:pb-2 text-gray-600 mr-0 inline-block whitespace-nowrap text-sm uppercase font-bold p-4 px-0"
          >
            AIBR KARAZ
          </Link>
          {/* User */}
          <ul className="md:hidden items-center flex flex-wrap list-none">
            <li className="inline-block relative">
              <NotificationDropdown />
            </li>
            <li className="inline-block relative">
              <UserDropdown />
            </li>
          </ul>
          {/* Collapse */}
          <div
            className={
              "md:flex md:flex-col md:items-stretch md:opacity-100 md:relative md:mt-4 md:shadow-none shadow absolute top-0 left-0 right-0 z-40 overflow-y-auto overflow-x-hidden h-auto items-center flex-1 rounded " +
              collapseShow
            }
          >
            {/* Collapse header */}
            <div className="md:min-w-full md:hidden block pb-4 mb-4 border-b border-solid border-gray-200">
              <div className="flex flex-wrap">
                <div className="w-6/12">
                  <Link
                    href="/"
                    className="md:block text-left md:pb-2 text-gray-600 mr-0 inline-block whitespace-nowrap text-sm uppercase font-bold p-4 px-0"
                  >
                    AIBR KARAZ
                  </Link>
                </div>
                <div className="w-6/12 flex justify-end">
                  <button
                    type="button"
                    className="cursor-pointer text-black opacity-50 md:hidden px-3 py-1 text-xl leading-none bg-transparent rounded border border-solid border-transparent"
                    onClick={() => setCollapseShow("hidden")}
                  >
                    <i className="fas fa-times"></i>
                  </button>
                </div>
              </div>
            </div>
            {/* Form */}
            <form className="mt-6 mb-4 md:hidden">
              <div className="mb-3 pt-0">
                <input
                  type="text"
                  placeholder="Search"
                  className="px-3 py-2 h-12 border border-solid  border-gray-500 placeholder-gray-300 text-gray-600 bg-white rounded text-base leading-snug shadow-none outline-none focus:outline-none w-full font-normal"
                />
              </div>
            </form>

            {/* Divider */}
            <hr
              className="my-4 md:min-w-full"
              style={{ border: "1px solid black" }}
            />
            {/* Heading */}
            <h6 className="md:min-w-full text-gray-500 text-xs uppercase font-bold block pt-1 pb-4 no-underline">
              ADMINISTRACION
            </h6>
            {/* Navigation */}

            <ul className="md:flex-col md:min-w-full flex flex-col list-none">
              <li className="items-center">
                <Link
                  href="/dashboard"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.asPath === "/dashboard"
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-tv mr-2 text-sm text-gray-800" +
                      (router.asPath === "/dashboard"
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  {console.log(router.pathname)}
                  Dashboard
                </Link>
              </li>

              <li className="items-center">
                <Link
                  href="/dashboard/settings"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.pathname.indexOf("/dashboard/settings") !== -1
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-tools mr-2 text-sm text-gray-800" +
                      (router.pathname.indexOf("/dashboard") !== -1
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  Settings
                </Link>
              </li>

              <li className="items-center">
                <Link
                  href="/dashboard/tables"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.pathname.indexOf("/dashboard/tables") !== -1
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-table mr-2 text-sm text-gray-800" +
                      (router.pathname.indexOf("/dashboard") !== -1
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  Tables
                </Link>
              </li>

              <li className="items-center">
                <Link
                  href="/dashboard/administration/projects"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.pathname.indexOf(
                      "/dashboard/administration/projects"
                    ) !== -1
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-building fa mr-2 text-sm text-gray-800" +
                      (router.pathname.indexOf(
                        "/dashboard/administration/projects"
                      ) !== -1
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  Proyectos
                </Link>
              </li>
              <li className="items-center">
                <Link
                  href="/dashboard/administration/investors"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.pathname.indexOf(
                      "/dashboard/administration/investors"
                    ) !== -1
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-money-bill mr-2 text-sm text-gray-800" +
                      (router.pathname.indexOf(
                        "/dashboard/administration/investors"
                      ) !== -1
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  Inversionistas
                </Link>
              </li>
              <li className="items-center">
                <Link
                  href="/dashboard/administration/users"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.pathname.indexOf(
                      "/dashboard/administration/users"
                    ) !== -1
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-user mr-2 text-sm text-gray-800" +
                      (router.pathname.indexOf(
                        "/dashboard/administration/users"
                      ) !== -1
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  Usuarios
                </Link>
              </li>
            </ul>

            {/* Divider */}
            <hr
              className="my-4 md:min-w-full"
              style={{ border: "1px solid black" }}
            />
            {/* Heading */}
            <h6 className="md:min-w-full text-gray-500 text-xs uppercase font-bold block pt-1 pb-4 no-underline">
              Ventas
            </h6>
            {/* Navigation */}

            <ul className="md:flex-col md:min-w-full flex flex-col list-none md:mb-4">
              <li className="items-center">
                <Link
                  href="/dashboard/sales/clients"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.pathname.indexOf("/dashboard/sales/clients") !== -1
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-users mr-2 text-sm text-gray-800" +
                      (router.pathname.indexOf("/dashboard/sales/clients") !==
                      -1
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  Clientes
                </Link>
              </li>

              <li className="items-center">
                <Link
                  href="/dashboard/sales/sales"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.pathname.indexOf("/dashboard/sales/sales") !== -1
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-cash-register mr-2 text-sm text-gray-800" +
                      (router.pathname.indexOf("/dashboard/sales/sales") !== -1
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  Ventas
                </Link>
              </li>
              <li className="items-center">
                <Link
                  href="/dashboard/sales/sellers"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.pathname.indexOf("/dashboard/sales/sellers") !== -1
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-user-tie mr-2 text-sm text-gray-800" +
                      (router.pathname.indexOf("/dashboard/sales/sellers") !==
                      -1
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  Vendedores
                </Link>
              </li>
            </ul>

            {/* Divider */}
            <hr
              className="my-4 md:min-w-full"
              style={{ border: "1px solid black" }}
            />
            {/* Heading */}
            <h6 className="md:min-w-full text-gray-500 text-xs uppercase font-bold block pt-1 pb-4 no-underline">
              Contable
            </h6>
            {/* Navigation */}

            <ul className="md:flex-col md:min-w-full flex flex-col list-none md:mb-4">
              <li className="items-center">
                <Link
                  href="/dashboard/accounting/envoices"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.pathname.indexOf(
                      "/dashboard/accounting/envoices"
                    ) !== -1
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-receipt mr-2 text-sm text-gray-800" +
                      (router.pathname.indexOf(
                        "/dashboard/accounting/envoices"
                      ) !== -1
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  Facturas
                </Link>
              </li>

              <li className="items-center">
                <Link
                  href="/dashboard/accounting/summary"
                  className={
                    "text-xs uppercase py-3 font-bold block " +
                    (router.pathname.indexOf(
                      "/dashboard/accounting/summary"
                    ) !== -1
                      ? "text-lightBlue-500 hover:text-lightBlue-600"
                      : "text-gray-700 hover:text-gray-500")
                  }
                >
                  <i
                    className={
                      "fas fa-file-invoice mr-2 text-sm text-gray-800" +
                      (router.pathname.indexOf(
                        "/dashboard/accounting/summary"
                      ) !== -1
                        ? "text-lightBlue-500 hover:text-lightBlue-600"
                        : "text-gray-700 hover:text-gray-500")
                    }
                  ></i>{" "}
                  Resumen contable
                </Link>
              </li>
            </ul>
          </div>
        </div>
      </nav>
    </>
  );
}
